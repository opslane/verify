...
accept() { # $1 = name, $2 = raw output
  ...
  echo "$clean" | jq --arg r "$1" '. + {reviewer: $r}' > "$RUN_DIR/review.json"
}
...
if [ "$DONE" = "0" ]; then
  ...
  jq -n '{reviewer: "unavailable", criteria: [], missing: []}' > "$RUN_DIR/review.json"
  exit 0
fi

echo "✓ Second opinion ($(jq -r '.reviewer' "$RUN_DIR/review.json")) → $RUN_DIR/review.json"
jq -r '.criteria[] | "  \(.id): \(.keep) — \(.why)"' "$RUN_DIR/review.json"
jq -r '.missing[]? | "  missing: \(.)"' "$RUN_DIR/review.json"
