...
for key in mode compose_file boot teardown seed seed_data_files health_url base_url env_file observe auth probes; do
  echo "$EXAMPLE" | jq -e --arg k "$key" 'has($k)' > /dev/null || { echo "FAIL: example missing key $key"; exit 1; }
done
echo "$EXAMPLE" | jq -e '.auth | keys == ["header","value_env"]' > /dev/null \
  || { echo "FAIL: auth must contain exactly header/value_env"; exit 1; }
...
grep -A4 -i 'How do API requests authenticate' "$SKILL" | grep -qi 'never written' \
  || { echo "FAIL: auth interview must say credential values are never written"; exit 1; }
